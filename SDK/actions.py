...
class ActionCatalog:
    ...
    def _tower_type_fit(self, state: GameState, player: int, x: int, y: int, tower_type: TowerType) -> float:
        ...
        if tower_type == TowerType.HEAVY:
            base = brute * 0.95 + local_density * 0.45 - forward_distance * 0.08
        elif tower_type == TowerType.HEAVY_PLUS:
            base = brute * 1.2 + protected * 0.35 + local_density * 0.4 - forward_distance * 0.04
        elif tower_type == TowerType.ICE:
            base = control * 1.15 + progress * 0.9 + randomized * 0.35 - protected * 0.7
        elif tower_type == TowerType.CANNON:
            base = control * 1.3 + progress * 1.35 + brute * 0.4 - protected * 1.0
        elif tower_type == TowerType.QUICK:
            base = swarm * 0.9 + 2.2 + progress * 0.2
        elif tower_type == TowerType.QUICK_PLUS:
            base = swarm * 1.0 + 3.0 + progress * 0.2
        elif tower_type == TowerType.DOUBLE:
            base = swarm * 1.25 + control * 0.2 + 3.5
        elif tower_type == TowerType.SNIPER:
            base = max(0.0, 18 - forward_distance) + brute * 0.45 + protected * 0.45
        elif tower_type == TowerType.MORTAR:
            base = swarm * 1.0 + max(0.0, 12 - forward_distance) + progress * 0.35
        elif tower_type == TowerType.MORTAR_PLUS:
            base = swarm * 1.1 + max(0.0, 13 - forward_distance) + brute * 0.2
        elif tower_type == TowerType.PULSE:
            base = swarm * 1.4 + control * 0.75 + progress * 0.4 - protected * 0.9
    ...
    def _tower_branch_key(self, tower_type: TowerType) -> str:
        ...
        if value in (1, 11, 12, 13):
            return "heavy"
        if value in (2, 21, 22, 23):
            return "quick"
        if value in (3, 31, 32, 33):
            return "mortar"
        return "other"

    def _tower_diversity_adjustment(self, state: GameState, player: int, tower_type: TowerType) -> float:
        branch = self._tower_branch_key(tower_type)
        branch_count = 0
        same_type_count = 0
        for tower in state.towers_of(player):
            if self._tower_branch_key(tower.tower_type) == branch:
                branch_count += 1
        ...
        adjustment = 0.0
        if branch == "quick":
            adjustment -= max(0, branch_count - 1) * 1.15
            adjustment -= same_type_count * 0.3
        elif branch in {"heavy", "mortar"}:
            if branch_count == 0:
                adjustment += 1.25
            else:
